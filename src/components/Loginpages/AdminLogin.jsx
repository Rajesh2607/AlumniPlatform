...
function AdminLogin() {
  ...
  const [in_password, setPassword] = useState("");
  ...
  const loginHandler = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/admin/admin-login`, {
        ...
        body: JSON.stringify({ in_email, in_password }),
      });
      ...
      if (response.ok) {
        ...
        setEmail("")
        setPassword("")
        navigate(`/admin/${data.adminId}`, { replace: true });
  ...
  return (
    ...
    <div className="md:w-1/2 max-md:mt-8 w-full">
      <h1 className="sm:text-3xl text-2xl font-bold font-Montserrat text-center">Admin Login</h1>
      <form onSubmit={loginHandler} className="flex flex-col my-4 gap-8">
        ...
        <div className="">
          Don't have an account? <Link to="/admin-register" className="font-medium text-celectic-blue">Register</Link>
        </div>
      </form>
    </div>
    </div>
